...
from ableton.v2.control_surface import BankingInfo, DeviceBankRegistry, DeviceDecoratorFactory, ControlSurface, InputControlElement, Layer, MIDI_CC_TYPE, PercussionInstrumentFinder
...
class ComradeMidi(ControlSurface):
    ...
    def _create_session(self):
        self._session_ring = SessionRingComponent(is_enabled=False, num_tracks=SESSION_WIDTH, num_scenes=SESSION_HEIGHT, tracks_to_use=lambda : tuple(self.song.visible_tracks) + tuple(self.song.return_tracks) + (
         self.song.master_track,), name='Session_Ring')
